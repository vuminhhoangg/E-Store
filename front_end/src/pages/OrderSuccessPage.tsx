...
const OrderSuccessPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            if (!id) {
                // Nếu không có ID, không cần tải thông tin đơn hàng
                console.log('OrderSuccessPage: Không có ID đơn hàng, hiển thị trang thành công chung');
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                if (response && response.data) {
                    console.log('OrderSuccessPage: Tải thông tin đơn hàng thành công:', response.data);
                    // Cấu trúc phản hồi có thể là { data: { ... } } hoặc chỉ là { ... }
                    const orderData = response.data.data || response.data;
                    setOrderDetails(orderData);
                } else {
                    console.error('OrderSuccessPage: Phản hồi không chứa dữ liệu đơn hàng');
                    setError('Không thể tải thông tin đơn hàng. Cấu trúc phản hồi không đúng.');
                }
            } catch (error) {
                console.error('OrderSuccessPage: Lỗi khi tải thông tin đơn hàng:', error);
    ...
    // Nếu không có ID đơn hàng hoặc không lấy được chi tiết đơn hàng, hiển thị thông báo đơn giản
    if (!id || !orderDetails) {
        return (
                        ...
                        <div className="p-8 text-center">
                            ...
                            <div className="border-t border-b border-gray-200 py-6 mb-6">
                                <p className="text-gray-700">
                                    Nhân viên của chúng tôi sẽ liên hệ với bạn để xác nhận đơn hàng và thông báo thời gian giao hàng dự kiến.
                                </p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <Link to="/orders" className="text-white bg-blue-600 hover:bg-blue-700 py-3 px-6 rounded-lg transition-colors flex items-center justify-center font-medium">
                                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
